...
const HttpManager = {
  // 获取图片信息
  attachImageUrl: (url) => `${getBaseURL()}/${url}`,//"https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png"
  // =======================> 用户 API 完成
  // 登录
  signIn: ({UserName,Password}) => post(`/api/userLogin`, {UserName,Password}),
  // 注册
  SignUp: ({UserName,Password,Sex,Phone_Num,Email,Birth,Introduction,Location}) => post(`user/add`, {UserName,Password,Sex,Phone_Num,Email,Birth,Introduction,Location}),
  // 删除用户
  deleteUser: (id) => get(`user/delete?id=${id}`),
  // 更新用户信息
  updateUserMsg: ({Id,UserName,Sex,Phone_Num,Email,Birth,Introduction,Location}) => post(`user/update`, {Id,UserName,Sex,Phone_Num,Email,Birth,Introduction,Location}),
